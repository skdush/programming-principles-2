...
class MickeysClock:
    def __init__(self, screen, width, height):
        ...
        # Загружаем изображение руки Микки, если есть
    ...
    def draw_clock_face(self):
        ...
        # Метки минут (60 делений)
        for i in range(60):
            angle = math.radians(i * 6 - 90)
            if i % 5 == 0:
                # Длинные метки для часов
                inner = self.radius - 20
                outer = self.radius - 5
                width = 3
            else:
                # Короткие метки для минут
    ...
    def _get_angle(self, value, max_value):
        """Вычисляет угол стрелки в радианах"""
        # 0 = наверх (-90 градусов), по часовой стрелке
        return math.radians(value / max_value * 360 - 90)

    def _draw_hand_with_image(self, image, angle, length):
        """Рисует стрелку как изображение (руку Микки)"""
        # Масштабируем изображение под нужную длину
        ...
        # Изображение нарисовано пальцами вверх.
        # pygame.rotate крутит против часовой, angle считается от оси X.
        # Чтобы "вверх" (angle=-π/2) → 0° поворота: degrees = -deg(angle) - 90
        degrees = -math.degrees(angle) - 90
        rotated = pygame.transform.rotate(scaled, degrees)

        # Центр blit-а смещён на длину/2 от центра в направлении angle
        # — тогда основание руки будет в центре циферблата
    ...
    def _draw_hand_line(self, angle, length, color, width):
        """Рисует стрелку как линию (запасной вариант)"""
    ...
    def draw(self):
        ...
        # Угол минутной стрелки (правая рука Микки)
        min_angle = self._get_angle(minutes + seconds / 60, 60)
        # Угол секундной стрелки (левая рука Микки)
        ...
        if self.hand_image:
            # Используем изображение руки Микки
            ...
            # Рисуем стрелки линиями (если нет изображения)
